...
public class CsvTradesLoader {
    ...
    public static BarSeries loadBitstampSeries() {
        ...
        InputStream stream = CsvTradesLoader.class.getClassLoader().getResourceAsStream("data/bitstamp_trades_from_20131125_usd.csv");
        List<String[]> lines = null;
        if(stream == null) {
            log.error("Data stream is null");
            throw new RuntimeException("CsvTradesLoader: Data stream is null");
        }
        try (CSVReader csvReader = new CSVReader(new InputStreamReader(stream, StandardCharsets.UTF_8))) {
            lines = csvReader.readAll();
            lines.remove(0); // Removing header line
        } catch (IOException | CsvException exception) {
            log.error("Unable to load trades from CSV", exception);
        }
